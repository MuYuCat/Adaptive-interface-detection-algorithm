...
dataFilePath =  uigetdir('Select unziped 3D.data path');     %选择数据文件夹
tic
...
%进行计算皮肤样本厚度、粗糙度的程序
          [surfaceLocation,depth,r] = skinDataProcessV4(fileName); % 只进行下表面相邻峰值取舍的优化算法 
...
%           [surfaceLocation,bottomLocation,surfaceIntensityA,bottomIntensityA,depth,r] = skinDataProcessV2(fileName); % 边界优化+上下峰值计算
          ...
          [filtSurface,SurfaceTrue] = saveSkinDataImageV2(surfaceLocation.surfaceLocation,depth.depth,r.r,resultDir,dataSubPath(i).name);
